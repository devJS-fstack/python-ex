...
class ImageComparisonApp:
    def __init__(self, root):
        ...
        self.comparison_method_label = tk.Label(root, text="Select Comparison Method:")
        ...
        for i, method in enumerate(self.comparison_methods):
            tk.Radiobutton(root, text=method, variable=self.method_var, value=method).grid(row=3+i, column=0, columnspan=2)
    ...
    def load_image1(self):
        ...
        if file_path:
            self.image1 = qt_cv2.imread(file_path)
    ...
    def load_image2(self):
        file_path = filedialog.askopenfilename()
        if file_path:
            self.image2 = qt_cv2.imread(file_path)
    ...
    def compare_images(self):
        ...
        hist1 = qt_cv2.calcHist([self.image1], [0], None, [256], [0, 256])
        hist2 = qt_cv2.calcHist([self.image2], [0], None, [256], [0, 256])
        hist_diff = qt_cv2.compareHist(hist1, hist2, qt_cv2.HISTCMP_CORREL)
        ...
        self.result_label.config(text=f"{color_comparison_result}\n{size_comparison_result}")


    def show_image(self, image, label):
        if image is not None:
            image = qt_cv2.cvtColor(image, qt_cv2.COLOR_BGR2RGB)
            image = qt_cv2.resize(image, (300, 300)) 
            photo = self.convert_image_to_photo(image)
            label.config(image=photo)
            label.image = photo

    def convert_image_to_photo(self, image):
        _, image_data = qt_cv2.imencode(".png", image)
